...
> 状态：实施中（spec 定稿 v2.1；**S1 导航 + S2 行情板块 + S3 合规/CI + S4 钱包(密钥核心+onboarding UI) + S5 交易(执行/签名层+下单UI)** 已完成；**UI 重构：单线示波器图标系统 + 7 屏对齐设计稿（行情/详情/交易/持仓/策略/钱包）已完成**、166 单测通过、tsc 零错、模拟器实时渲染无运行时错误；最终真机部署(需 Apple 签名)与真实 testnet 下单(需充值钱包+生物识别确认)为人工交互步骤）。
...
| 1 | 只读行情（市场列表/详情/view-only 持仓） | **行情列表/详情/持仓 UI ✅**（Markets/Detail/Positions 屏均已实现并对齐设计稿；166 单测通过，Metro 打包通过） |
